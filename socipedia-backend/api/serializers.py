...
class PostSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        # Check image for inappropriate content if picture is provided
        if picture:
            try:
                from .image_moderation import should_block_image
                is_blocked, reasons, details = should_block_image(picture)
                ...
                if is_blocked:
                    ...
                    error_data = {
                        ...
                        "image_moderation": {
                            "reasons": reasons,
                            "nudity_detected": details.get('nudity', {}).get('detected', False),
                            "nudity_confidence": details.get('nudity', {}).get('confidence', 0.0),
                            "hate_speech_detected": details.get('hate_speech', {}).get('detected', False),
                            "hate_keywords": details.get('hate_speech', {}).get('keywords', []),
                        }
                    }
                ...
                # If image moderation is not available, log warning but don't block
                import logging
                logging.warning(f"Image moderation not available: {str(e)}")
            except Exception as e:
                # Log error but don't block post creation for other errors
                import logging
