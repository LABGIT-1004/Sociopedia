...
const MyPostWidget = ({ picturePath }) => {
  ...
  const handlePost = async () => {
    try {
      ...
      // Check if the response indicates blocked content (description or picture)
      if (
        newPost &&
        (Array.isArray(newPost.description) || Array.isArray(newPost.picture))
      ) {
        // Show specific error message
        if (Array.isArray(newPost.description)) {
          toast.error(newPost.description[0]);
        } else if (Array.isArray(newPost.picture)) {
          toast.error(newPost.picture[0]);
        }
        return; // Don't proceed with creating the post
      }
          ...
          >
            {({ getRootProps, getInputProps }) => (
              <FlexBetween>
                ...
                {image && (
                  <IconButton onClick={removeImage} sx={{ width: "15%" }}>
                    <DeleteOutlined />
                  </IconButton>
                )}
              </FlexBetween>
