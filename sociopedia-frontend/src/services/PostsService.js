...
class PostsService {
  ...
  static async createPost(formData, token) {
    try {
      ...
      // Check if response indicates blocked content (400 status with description or picture array)
      if (response.status === 400 && response.data) {
        if (Array.isArray(response.data.description) || Array.isArray(response.data.picture)) {
      ...
      if (error.response && error.response.status === 400 && error.response.data) {
        if (Array.isArray(error.response.data.description) || Array.isArray(error.response.data.picture)) {
